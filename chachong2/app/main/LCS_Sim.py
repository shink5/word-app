...
def sim_total(*file_path):  # 输入两个文件或一个文件夹，若为文件夹返回（文件相似度矩阵，自适应阈值），若为两个文件，返回相似度
    ...
    if len(file_path) == 1:
        ...
        matrix = pd.DataFrame([], index=files, columns=files)##是否初始化零了,初始化矩阵
        for file1 in files:
            for file2 in files:
        ...
        for file in files:
            pre_path = file_path[0] + "\\pre\\pre_" + file
            path = file_path[0] + "\\" + file
            pretreatment(path,pre_path)

        for file1 in files:
            for file2 in files:
                if file1 != file2 and matrix[file1][file2] == 0 and matrix[file2][file1] == 0:
                    pre_path1 = file_path[0] + "\\pre\\pre_" + file1
                    pre_path2 = file_path[0] + "\\pre\\pre_" + file2
                    sim_value = round(sim(pre_path1, pre_path2),3)
                    matrix[file1][file2] = sim_value
                    matrix[file2][file1] = sim_value

        return (matrix, 0.8)  #固定阈值

    if len(file_path) == 2:
        path1 = file_path[0]
        path2 = file_path[1]
        pre_path1 = os.path.dirname(file_path[0]) + "\\pre" + r"\pre_" + os.path.basename(file_path[0])
        pre_path2 = os.path.dirname(file_path[1]) + "\\pre" + r"\pre_" + os.path.basename(file_path[1])
        pretreatment(path1,pre_path1)
        pretreatment(path2,pre_path2)
        return sim(pre_path1, pre_path2)
